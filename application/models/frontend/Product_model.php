...
class Product_model extends CI_Model
{
    ...
    public function recommentProductByPrice($price = 0, $offset = 100) {
        $sql = "SELECT product_id, product_url, product_title, product_thumb, product_price, product_desc, product_buy, product_view FROM product WHERE product_price <= " . ($price + $offset) . " AND product_price >= " . ($price - $offset) . "  ORDER BY product_id DESC LIMIT " . MAX;
    ...
    public function recommentProductByUser() {
        $sql = "SELECT product_id, product_url, product_title, product_thumb, product_price, product_desc, product_buy, product_view FROM product ORDER BY product_view DESC LIMIT " . MAX;
        $result = $this->db->query($sql)->result();
        foreach ($result as $key) {
            echo '<div class="col-sm-12">
                                                <div class="box">
